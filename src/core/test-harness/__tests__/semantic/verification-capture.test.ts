...
const VERIFICATION_FIXTURE: TestFixture = {
  nodes: [
    { labels: ['VerificationRun'], properties: { name: 'done-check-run-1', runId: 'vr:test:1', ranAt: '2026-01-01T00:00:00.000Z', status: 'pass', toolName: 'done-check' }, ref: 'vr' },
    { labels: ['GateDecision'], properties: { name: 'gate-decision-1', decision: 'pass', decisionHash: 'sha256:abc123' }, ref: 'gate' },
    { labels: ['CommitSnapshot'], properties: { name: 'commit-snap-1', headSha: 'abc123def456', branch: 'main' }, ref: 'commit' },
    { labels: ['WorkingTreeSnapshot'], properties: { name: 'wt-snap-1', isDirty: false, diffHash: 'sha256:000' }, ref: 'wt' },
    { labels: ['Artifact'], properties: { name: 'integrity-snapshot', artifactPath: 'artifacts/integrity-snapshots/2026-01-01.jsonl', artifactHash: 'sha256:fff' }, ref: 'artifact' },
  ],
  edges: [
...
describe('N4: Verification Capture Chain', () => {
  it('verification run links to all artifacts', async () => {
    ...
    expect(types).toContain('CAPTURED_COMMIT');
    expect(types).toContain('CAPTURED_WORKTREE');
    expect(types).toContain('EMITS_GATE_DECISION');
    expect(types).toContain('GENERATED_ARTIFACT');
    await ctx.finish();
  });

  it('missing gate decision is detectable', async () => {
  ...
  it('replay: verification fixture deterministic', async () => {
    const config = {
  ...
  it('idempotent: capture queries stable', async () => {
    const ctx = await setupReplay({
    ...
    expect(r1.records[0].get('edges').toNumber()).toBe(r2.records[0].get('edges').toNumber());
    await ctx.finish();
  });
});

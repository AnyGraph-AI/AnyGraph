...
const WAIVER_FIXTURE: TestFixture = {
  nodes: [
    ...
    { labels: ['Task'], properties: {
      name: 'Active waiver task', status: 'done', hasCodeEvidence: false,
      waiverReason: 'Dependency not available yet', waiverExpiresAt: '2026-06-01T00:00:00.000Z', waiverApprovedBy: 'jonathan',
    }, ref: 'task_active_waiver' },
    { labels: ['Task'], properties: {
      name: 'Expired waiver task', status: 'done', hasCodeEvidence: false,
      waiverReason: 'Legacy code not testable', waiverExpiresAt: '2025-01-01T00:00:00.000Z', waiverApprovedBy: 'jonathan',
    }, ref: 'task_expired_waiver' },
    { labels: ['Task'], properties: { name: 'No waiver task', status: 'done', hasCodeEvidence: true }, ref: 'task_no_waiver' },
  ],
  edges: [
...
describe('N4: Waiver/Expiry Enforcement', () => {
  it('active waiver passes enforcement', async () => {
    ...
    expect(result.records[0].get('name')).toBe('Active waiver task');
    await ctx.finish();
  });

  it('expired waiver fails enforcement', async () => {
    const ctx = await setupReplay({
    ...
    `, { pid: ctx.graph!.projectId });
    expect(result.records).toHaveLength(1);
    expect(result.records[0].get('name')).toBe('Expired waiver task');
    await ctx.finish();
  });

  it('replay: waiver check deterministic', async () => {
    const config = {
  ...
  it('idempotent: waiver query stable', async () => {
    const ctx = await setupReplay({
    ...
    expect(r1.records[0].get('c').toNumber()).toBe(r2.records[0].get('c').toNumber());
    await ctx.finish();
  });
});

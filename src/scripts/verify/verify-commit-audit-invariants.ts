...
const ROADMAP_LINKS: Record<InvariantKey, Array<{ task: string; line: number }>> = {
  ...
  audit_working_tree_policy: [
    {
      task: 'Enforce forensic audit policy for dirty worktrees in commit audit runner',
      line: 1237,
    },
  ],
  audit_profile_contract: [
    {
      task: 'Introduce model-agnostic audit profile spec and enforce required sections/commands contract',
      line: 1244,
    },
  ],
};
...
function getWorkingTreeDelta(): WorkingTreeDelta {
  ...
  return {
    ...
    trackedSample: tracked.slice(0, 20),
    untrackedSample: untracked.slice(0, 20),
  };
}
...
async function checkS6BaselineOutputContract(): Promise<InvariantResult> {
  const verifyPath = join(process.cwd(), 'src', 'scripts', 'verify', 'verify-graph-integrity.ts');
...
function checkAuditProfileContract(): InvariantResult {
  const doc = readLocalFile('docs/AUDIT_PROFILE_STANDARD.md');
  ...
  const ok = missingCommands.length === 0 && missingSections.length === 0;

  return {
    key: 'audit_profile_contract',
    ok,
    summary: ok
      ? 'Audit profile contract passed.'
      : `Audit profile contract failed (missingCommands=${missingCommands.length}, missingSections=${missingSections.length}).`,
    details: {
      profilePath: 'docs/AUDIT_PROFILE_STANDARD.md',
      missingCommands,
      missingSections,
    },
  };
...
function checkAuditWorkingTreePolicy(
  commitCount: number,
  workingTree: WorkingTreeDelta,
  allowDirty: boolean,
): InvariantResult {
  const zeroCommitDirty = commitCount === 0 && workingTree.dirty;
  const ok = !(zeroCommitDirty && !allowDirty);

  return {
    key: 'audit_working_tree_policy',
    ok,
    summary: ok
      ? 'Working-tree audit policy satisfied.'
      : 'Audit range has 0 commits but working tree is dirty (set COMMIT_AUDIT_ALLOW_DIRTY=true only for explicit override runs).',
    details: {
      commitCount,
      dirty: workingTree.dirty,
      trackedCount: workingTree.trackedCount,
...
async function main(): Promise<void> {
  ...
  const workingTree = getWorkingTreeDelta();
  const allowDirty = process.env.COMMIT_AUDIT_ALLOW_DIRTY === 'true';
  ...
  try {
    ...
    invariants.push(checkAuditProfileContract());
    invariants.push(checkAuditWorkingTreePolicy(commitCount, workingTree, allowDirty));
    ...
    const report: CommitAuditReport = {
      ...
      changedFiles,
      workingTree,
      dirtyOverrideUsed: allowDirty && workingTree.dirty,
      invariants,
    ...
    console.log(
      JSON.stringify({
        ...
        workingTreeDirty: workingTree.dirty,
        workingTreeTracked: workingTree.trackedCount,
        workingTreeUntracked: workingTree.untrackedCount,
        dirtyOverrideUsed: report.dirtyOverrideUsed,
        failingInvariantKeys,
        confidence,
